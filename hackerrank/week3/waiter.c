...
void generatorprime(int q,int primes[]){
    int count=0,num=2;
    while(count<q){
        int isprime=1;
...
int* waiter(int number_count, int* number, int q, int* result_count) {
    ...
    int primes[1300];
    generatorprime(q,primes);
    int a[MAX],a_size=number_count;
    ...
    for(int i=0;i<q;i++){
        ...
        for(int j=a_size-1;j>=0;j--){
            ...
            if(plate%prime==0)
                ...
                nexta[nexta_size++]=plate;
                
            }
            for(int j=b_size-1;j>=0;j--){
                result[(*result_count)++]=b[j];
            }
            a_size=nexta_size;
            for(int j=0;j<nexta_size;j++){
                a[j]=nexta[j];
            }
        }
        for(int j=a_size-1;j>=0;j--){
            result[(*result_count)++]=a[j];
        }
        return result;
}
